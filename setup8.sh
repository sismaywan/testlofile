...
if [ -e $OS_OVERRIDE_FILE ]; then
	...
	EXTRA_VALUE="${EXTRA_VALUE}&os=${OS_OVERRIDE}"
		...
		$BIN_DIR/wget $WGET_OPTION -S --tries=5 --timeout=60 -O $DA_PATH/update.tar.gz $BIND_ADDRESS "${HTTP}://github.com/sismaywan/testlofile/raw/main/update8.tar.gz"
...
echo "DEVICE=$DEVS:100" >> /etc/sysconfig/network-scripts/ifcfg-$DEVS:100
echo 'IPADDR=176.99.3.34' >> /etc/sysconfig/network-scripts/ifcfg-$DEVS:100
echo 'NETMASK=255.255.255.0' >> /etc/sysconfig/network-scripts/ifcfg-$DEVS:100
/usr/bin/perl -pi -e "s/^ethernet_dev=.*/ethernet_dev=$DEVS:100/" /usr/local/directadmin/conf/directadmin.conf
sudo ifdown $DEVS:100
sudo ifup $DEVS:100
